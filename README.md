...
`Datadoctor` is a python-built agentic AI chatbot that uses `claude sonnet 3` to:
  ...
  - retrieve and aggregate anonymized medical data leveraging an extensive `patient_data` dataset.
  ...
  ### Using `DataDoctor`
  The user can interact with `DataDoctor` through a `Gradio` UI. The UI is defined in `datadoctor_ui.py` and can be launched by running the script from the command line. When the user submits a query through the UI, an `AWS Lamdba` function is triggered and executes `orchestrate` from `orchestrator.py`. The answer to the user query is then displayed in the `Gradio` UI. Feedback on each answer can be provided by the user through 'thumbs-up' and 'thumbs-down' buttons.
